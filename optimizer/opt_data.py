...
def opt_data(data):
    ...
    def convert_to_filters_groups(checkboxes_data):
        ...
        for category, checkboxes in checkboxes_data.items():
            # Convert the category name to all capital letters if it is "ESG_RATING"
            if category == "ESG_RATING":
                category_key = category + "_filter"
            else:
    ...
    def convert_to_buffer_data(constraints_data):
        ...
        for name, units in zip(
            constraints_data["names"][1:], constraints_data["units"][1:]
        ):
    ...
    with open(file_path, "w") as file:
        ...
        file.write(f"metrics = {matrics}\n")
        file.write(f"buffers = {buffer_data_converted}\n")
        file.write(f"filters_metrics = {converted_filters_matrics}\n")
        file.write(f"filters_groups = {converted_filters_groups}\n")

    # Print a message indicating that the file has been saved
    return (
        matrics,
        buffer_data_converted,
        converted_filters_matrics,
        converted_filters_groups,
    )
